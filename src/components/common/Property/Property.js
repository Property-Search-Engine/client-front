...
export default function Property(props) {
    ...
    const handleClick = (e) => {
        ...
        if(isProperties) {
            setMain(false)
            setsinglePropertyId(e.currentTarget.id)
        }
    }
    ...
    return (
        <div className="propertyContainer"  onClick={handleClick} id={property._id}>
            ...
            <div>
                <div className="d-flex flex-row justify-content-between detailsContainer" id={id}>
                    <div className="details">
                        <p className="address">{number} {street}</p>
                        <p className="location">{city}</p>
                        <p className="price">${price}</p>
                    </div>
                    <div className="icon">
                        <img src={heartGrey ? "/assets/icons/like.svg" : "/assets/icons/like-red.svg"}></img>
                    </div>
                </div>
                <div className="d-flex flex-row propertyCharacteristics border-top">
                    <PropertyCharacteristics beds={bedRooms} baths={bathRooms} m2={surface}/>
                    <Contact/>
                </div>
            </div>
        </div>
